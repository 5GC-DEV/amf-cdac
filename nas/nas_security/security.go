...
func Decode(ue *context.AmfUe, accessType models.AccessType, payload []byte) (*nas.Message, error) {
	...
	if msg.SecurityHeaderType == nas.SecurityHeaderTypePlainNas {
		...
		ue.NASLog.Info("---ue.KnasInt: ", ue.KnasInt)
		ue.NASLog.Info("---ue.ULCount.Get(): ", ue.ULCount.Get())
		ue.NASLog.Info("---security.Bearer3GPP: ", security.Bearer3GPP)
		ue.NASLog.Info("---security.DirectionUplink: ", security.DirectionUplink)
		ue.NASLog.Info("---payload: ", payload)
